...
from models import tbl_hp as tbl_hpModel
...
class BaseMethod():
    ...
    def __init__(self):
        self.raw_weight = {'reputasi_brand': 3, 'processor': 5, 'baterai': 4, 'harga': 5, 'ukuran_layar': 2}
    ...
    @property
    def data(self):
        query = select(tbl_hpModel.nama_hp, tbl_hpModel.reputasi_brand, tbl_hpModel.processor, tbl_hpModel.baterai, tbl_hpModel.harga, tbl_hpModel.ukuran_layar)
        result = session.execute(query).fetchall()
        print(result)
        return [{'nama_hp': tbl_hp.nama_hp, 'reputasi_brand': tbl_hp.reputasi_brand, 'processor': tbl_hp.processor, 'baterai': tbl_hp.baterai, 'harga': tbl_hp.harga, 'ukuran_layar': tbl_hp.ukuran_layar} for tbl_hp in result]
...
class tbl_hp(Resource):
    ...
    def get(self):
        query = select(tbl_hpModel)
        data = [{'nama_hp': tbl_hp.nama_hp, 'reputasi_brand': tbl_hp.reputasi_brand, 'processor': tbl_hp.processor, 'baterai': tbl_hp.baterai, 'harga': tbl_hp.harga, 'ukuran_layar': tbl_hp.ukuran_layar} for tbl_hp in session.scalars(query)]
        return self.get_paginated_result('tbl_hp/', data, request.args), HTTPStatus.OK.value


api.add_resource(tbl_hp, '/tbl_hp')
